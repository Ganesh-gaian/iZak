...
function createDeviceCard(device) {
  ...
  statusCont.className = `statusCont ${device.id}`;

  const statusImg = document.createElement("img");
  statusImg.src = device.statusImg;
  statusImg.alt = "";

  if (device.id === "updateCarbonMono" || device.id === "updateCarbonMonoBg") {
  ...
  if (device.nextScheduled) {
    ...
    const image80px = document.createElement("img");
    image80px.src = "Quiet Zone.svg";
    image80px.className = "imgAboveTextManual";
    imageDiv.appendChild(image80px);
    dbRightCont.appendChild(imageDiv);

    const span120 = document.createElement("span");
    span120.id = "span120";
    span120.textContent = "Quiet Mode";
